'''
	arrayAbsurdityCodeEval.py - Solution to Problem Array Absurdity (Category - Medium)
...
this file is one test case. Ignore all empty lines. Each line begins with a positive 
integer(N) i.e. the size of the array, then a semicolon followed by a comma separated 
list of positive numbers ranging from 0 to N-2, inclusive. i.e 
...
if __name__ == '__main__':
	f = open(sys.argv[1], 'r')
	test_cases = (i.split(';')[1].split(',') for i in f.read().split('\n') if len(i) > 0)
	
	for test_case in test_cases:
		 print [i for i in test_case if test_case.count(i) > 1][0]
